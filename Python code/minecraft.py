...
# runloop.run(main())
    motor.run(port.C, 90,acceleration=10000)
    await runloop.sleep_ms(6500)
    motor.stop(port.C)

    # After solving
    # Move back    
    await motor_pair.move_for_degrees(motor_pair.PAIR_1, 350, 0)
    ...
    await motor.run_for_degrees(port.D, 500, 500)
     ...
     # Move back to Base
    await motor_pair.move_for_degrees(motor_pair.PAIR_1, 1800, 0)
